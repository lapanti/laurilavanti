...
### Context
A citations page presents endorsements from named individuals who vouch for Lauri. The page exists in all three locales but the citation text is always in Finnish — no translations are produced for the individual quotes. The page is intended to build trust with voters and supporters.
...
- **Page files (one per locale, same component, same data):**
  ...
  - `src/pages/sv/citations/index.mdx`
  - `src/pages/en/citations/index.mdx`
  All three import from `src/content/citations.ts` and render the same `Citations` component.

- **Component hierarchy:**
  ```
  Citations.astro              (grid/list wrapper, receives Citation[])
    └── Citation.astro         (one card per person)
          ├── CldImage         (portrait, square crop centred on face)
          └── blockquote       (citation text, name, title)
  ```
...
- **Nav:** add a nav link entry to `src/content/nav.ts` for all three locales pointing to `/{lang}/citations/`. Language-switch links with `switchToLang` are not needed for this page — the locale-prefix-swap script handles it automatically since all three pages share the same path segment `citations`.
...
- **Frontmatter schema:**
  ...
  slug: 'fi/citations'             # locale-prefixed, no trailing slash here
  type: 'WebPage'
...
- Do not add the citations page to the sitemap while `noindex` is active
...
- [ ] `/{lang}/citations/` renders correctly in all three locales
...
- All three locale pages must import from the same `src/content/citations.ts` — drift between locales is the primary risk
- Adding a `citations` nav link must be done for all three locales in `nav.ts` simultaneously — partial nav updates break the mobile/desktop menu symmetry
...
**Scenario: Citations page renders the same content in all locales**
- Given: `citations.ts` has 5 entries
- When: A visitor navigates to `/fi/citations/`, `/sv/citations/`, or `/en/citations/`
- Then: All three pages render the same 5 citation cards with Finnish citation text; the page title and surrounding UI chrome are in the respective locale language

**Scenario: Language switch from Finnish citations page**
- Given: A visitor is on `/fi/citations/`
- When: They click the Swedish language-switch nav link
- Then: The inline script swaps `/fi/` → `/sv/` and lands on `/sv/citations/`, which exists

**Scenario: New citation added**
- Given: A new `Citation` entry is appended to `citations.ts`
...
- Given: A `Citation` entry has `image: ''` or an invalid Cloudinary filename
...
- When: A search engine or AI crawler visits `/{lang}/citations/`
...
- Given: Explicit instruction to publish the citations page is received
...
**Scenario: Citation with long text**
- Given: A citation entry has a multi-paragraph `citation` string
