...
const turquoiseHex = '#25F4EE'
const redHex = '#FE2C55'

const TikTokComponent = ({ className, noHover }: Props) => {
    const [turquoise, setTurquoise] = useState(noHover ? turquoiseHex : colors.white)
    const [red, setRed] = useState(noHover ? redHex : colors.white)
    const [black, setBlack] = useState(noHover ? colors.black : colors.white)

    const onHover = useCallback(() => {
        if (!noHover) {
            setTurquoise(turquoiseHex)
            setRed(redHex)
            setBlack(colors.black)
        }
    }, [noHover, setTurquoise, setRed, setBlack])

    const onBlur = useCallback(() => {
        if (!noHover) {
            setTurquoise(colors.white)
            setRed(colors.white)
            setBlack(colors.white)
        }
    }, [noHover, setTurquoise, setRed, setBlack])
...
TikTokComponent.displayName = 'TikTok'

const TikTok = styled(TikTokComponent)({})
